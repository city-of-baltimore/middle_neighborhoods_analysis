...
```{r}
sales <- sales %>% mutate(full.street = paste0(`House #`, " ", `Street Name`, " ", Suffix  ))
```
...
```{r}
mris.recent %>%
  left_join(hmt.hood, by = c("Neighborhood" = "Neighborhood")) %>%
  group_by(`Predominant Code Ignoring Non-Residential`) %>%
  summarise(max.sale.price = max(Close_Price))
```
...
sales.mid.hoods.top3.geo <- SpatialPointsDataFrame(
  sales.mid.hoods.top3.geo %>% select(long, lat), 
...
sale.labels <- paste0(
  "Analysis: SDAT (through October 18)<br>",
  sales.mid.hoods.top3.geo$`House #`, " ",
...
mris.recent %>% 
  filter(Transaction_Type == "Standard") %>%
  group_by(Neighborhood) %>%
  arrange(desc(Close_Price)) %>%
  mutate(price.order = row_number()) %>%
  filter(price.order <= 3) %>%
  arrange(Neighborhood, price.order) 
```
...
plot_hood_sales_timeline <- function(hood){
  ...
  df <- sales %>% 
    filter(neighborhood == hood, 
           deed.date >= "2015-01-01",
           `How Conveyed` == 1,
           !grepl("NOT", rescode))
  
  g <- df %>%
    ggplot(aes(deed.date, `Sales Price`)) +
    geom_point(color = ifelse((df$full.street %in% sales.mid.hoods.top3$full.street), 
                              "red", "black"),
               aes(text = paste(`House #`, `Street Name`))) +
    theme_iteam_google_docs() +
    ylim(c(0, 500000))
