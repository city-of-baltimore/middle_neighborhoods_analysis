...
```{r}
notices %>% 
  mutate(notice.year = year(DateNotice)) %>%
  count(Status, notice.year) %>%
  ggplot(aes(notice.year, n)) +
...
```{r}
notices %>% filter(BlockLot == "2951D065 ")
```
```{r}
View(notices)
```
...
```{r}
vbn %>% 
  #filter(VBN_Status %in% c("New_Open", "New_Closed")) %>%
  mutate(year.issued = year(DateNotice)) %>%
  count(year.issued, VBN_Status) %>%
  ggplot(aes(year.issued, n)) +
  geom_col(aes(fill = VBN_Status)) +
  theme_iteam_presentations()
```

```{r}
vbn %>% 
  filter(VBN_Status == "Old_Open  ") %>%
  View
```
